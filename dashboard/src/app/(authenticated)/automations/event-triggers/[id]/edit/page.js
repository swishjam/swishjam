...
export default function EditTriggerPage({ params }) {
  ...
  useEffect(() => {
    ...
    SwishjamAPI.EventTriggers.retrieve(id).then(({ trigger, error }) => {
      setSelectedEventName(trigger.event_name)
      setSelectedSlackChannelId(trigger.steps[0].config.channel_id)
      setSlackMessageHeader(trigger.steps[0].config.message_header)
      setSlackMessageBody(trigger.steps[0].config.message_body)
      setConditionalStatements(trigger.conditional_statements)
      SwishjamAPI.Events.Properties.listUnique(trigger.event_name).then(setPropertyOptionsForSelectedEvent);
    })
  }, [id])
