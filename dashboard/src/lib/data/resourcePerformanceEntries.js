...
export default class ResourcePerformanceEntries {
  static async getTimeSeriesForUrl({ projectKey, url, urlHost, urlPath, startTs, percentile = 0.75 }) {
    if (urlHost && urlPath) {
      const query = `
        SELECT
          name,
          ...
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY start_time ASC) AS start_time,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY waiting_duration ASC) AS waiting_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY redirect_duration ASC) AS redirect_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY dns_lookup_duration ASC) AS dns_lookup_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY tcp_duration ASC) AS tcp_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY ssl_duration ASC) AS ssl_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY request_duration ASC) AS request_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY response_duration ASC) AS response_duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY duration ASC) AS duration,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY transfer_size ASC) AS transfer_size,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY decoded_body_size ASC) AS decoded_body_size,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY encoded_body_size ASC) AS encoded_body_size,
          PERCENTILE_CONT(${parseFloat(percentile)}) WITHIN GROUP (ORDER BY fetch_start ASC) AS fetch_start
        FROM
        ...
        WHERE
          ...
          name_to_url_host = $2 AND
          name_to_url_path = $3 AND
        ...
        ORDER BY
          day, hour
      `;
      return (await db.query(query, [projectKey, decodeURIComponent(urlHost), decodeURIComponent(urlPath), new Date(startTs)])).rows;
    } else {
      const query = `
        SELECT
          name,
  ...
  static async getTimeseriesForMetric({ projectKey, resourceName, resourceUrlHost, resourceUrlPath, metric, startTs }) {
    if (resourceUrlHost && resourceUrlPath) {
      const query = `
        SELECT
          rpe.name AS name,
        ...
        WHERE
          pv.project_key = $1 AND
          rpe.name_to_url_host = $2 AND
          rpe.name_to_url_path = $3 AND
          pv.page_view_ts >= $4 AND
        ...
        ORDER BY
          day, hour ASC
      `;
      return (await db.query(query, [projectKey, decodeURIComponent(resourceUrlHost), decodeURIComponent(resourceUrlPath), new Date(startTs)])).rows;
    } else {
      const query = `
        SELECT
          rpe.name AS name,
  ...
  static async getXHRResources({ projectKey, urlHost, urlPath, percentile = 0.75, startTs }) {
    if (!projectKey) throw new Error('projectKey is required');
    if (!urlHost) throw new Error('urlHost is required');
    if (!startTs) throw new Error('startTs is required');
    if (urlPath) {
      const query = `
        SELECT
          CONCAT(xhr.name_to_url_host, xhr.name_to_url_path) AS url,
          COUNT(xhr.name) AS total_count,
          PERCENTILE_CONT(${percentile}) WITHIN GROUP (ORDER BY xhr.duration ASC) AS duration,
          PERCENTILE_CONT(${percentile}) WITHIN GROUP (ORDER BY xhr.transfer_size ASC) AS transfer_size
        FROM
          resource_performance_entries AS xhr
        JOIN
          page_views AS pv ON xhr.page_view_uuid = pv.uuid
        WHERE
          pv.project_key = $1 AND
          pv.url_host = $2 AND
          pv.url_path = $3 AND
          pv.page_view_ts >= $4 AND
          xhr.initiator_type IN ('xmlhttprequest', 'fetch')
        GROUP BY
          url
        ORDER BY
          total_count DESC
      ...
      const query = `
        ...
        FROM
          resource_performance_entries  AS xhr
        JOIN
          page_views AS view ON xhr.page_view_uuid = view.uuid
        WHERE
          view.project_key = $1 AND
          view.url_host = $2 AND
          view.page_view_ts >= $3 AND
          xhr.initiator_type IN ('xmlhttprequest', 'fetch')
        GROUP BY
          url
        ORDER BY
          total_count DESC
      `;
      return (await db.query(query, [projectKey, decodeURIComponent(urlHost), new Date(startTs)])).rows; 
    }
  }
}