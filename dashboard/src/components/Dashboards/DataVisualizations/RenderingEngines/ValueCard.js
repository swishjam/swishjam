...
export default function ValueCardRenderingEngine({
  ...
  useEffect(() => {
    ...
    SwishjamAPI.Events.value(event, { property, query_groups: JSON.stringify(whereClauseGroups), aggregation_method: aggregationMethod, timeframe, dataSource, include_comparison })
  ...
  }, [event, property, aggregationMethod, timeframe, include_comparison, JSON.stringify(whereClauseGroups.map(group => group.queries))]);
