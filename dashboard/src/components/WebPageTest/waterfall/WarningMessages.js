...
export default function WarningMessage({ requestData, lcpImageURL, lcpValue }) {
  ...
  // const msFromTtfbToLcpDiscovery = lcpImageDiscoveredAt - requestData[0].firstTimestamp();
  ...
  const warnings = [
    ...
    !['image/webp', 'image/avif'].includes(lcpImageFormat)  && <span>The LCP image is being served in the {lcpImageFormat}, consider changing this to the <span className='font-bold'>webp</span> format. Webp images are smaller and faster to download than other image formats.</span>,
  ].filter(Boolean);
