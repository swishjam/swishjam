    ...
    >
      <button {...props} className={`group cursor-pointer w-fit inline-flex items-center justify-center px-1 rounded ${pressEffect ? `transform transition-transform duration-150 active:scale-[95%]` : ''} ${props.className || ''}`}>
        <div className='relative w-fit inline-block'>
          {children ?? <CopyIcon className={`h-4 w-4 transition-colors text-gray-700 group-hover:text-gray-900 group-hover:bg-gray-50 group-active:text-gray-900 group-active:bg-gray-50 ${props.copyIconClassName}`} />}
