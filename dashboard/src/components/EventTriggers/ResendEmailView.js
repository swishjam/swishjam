...
export default function ResendEmailView() {
  const router = useRouter();
  const { email: currentUserEmail } = useAuthData();
  const form = useForm({ defaultValues: { subject: '✨ Event Name', to: '{ user.email }' } });
  ...
  const setSelectedEventAndGetPropertiesAndAutofillMessageContentIfNecessary = async eventName => {
    form.setValue('subject', '✨ ' + eventName + ' ✨')
  ...
  async function onSubmit(values) {
    setLoading(true)
    const isValid = values.event_name && (values.subject || values.body)
    if (!isValid) {
      Object.keys(values).forEach(key => {
        if (!values[key]) {
          if ((key === 'subject' || key === 'body') && !values.subject && !values.body) {
            form.setError(key, { message: 'You must provide either a subject or a body value for your slack message.' })
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 mb-8">
      {testTriggerModalIsOpen && (
        <TestTriggerModal
          ...
          slackMessagesubject={form.watch('subject')}
          ...
          <EmailPreview
            to={
              <InterpolatedMarkdown
                ...
                availableEventOptions={propertyOptionsForSelectedEvent}
              />
            }
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormInputOrLoadingState isLoading={uniqueEvents === undefined}>
                        <Select
                ...
                <div className='mt-4'>
                  ...
                  <FormInputOrLoadingState className='h-24 mt-2' isLoading={uniqueEvents === undefined}>
                  ...
                  {!form.watch('event_name') || (!form.watch('subject') && !form.watch('body'))
                    ? (
                      ...
                      <button
                        ...
                        disabled={loading || uniqueEvents === undefined}
