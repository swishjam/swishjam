...
const initializeMarkdownParser = acceptableHighlightValues => {
  ...
  markdown.renderer.rules.highlight = function (tokens, idx) {
    ...
    if (isValidHighlight) {
      return `<span class="bg-gray-200 font-semibold italic text-gray-700 px-1 py-0.5 rounded-md">${content}</span>`;
    } else {
      ...
      return (
        `<span class="bg-red-200 font-semibold italic text-red-700 px-1 py-0.5 rounded-md inline-flex items-center w-fit gap-x-2">
          ${content} 
          ${warningIcon}
        </span>`
      )
