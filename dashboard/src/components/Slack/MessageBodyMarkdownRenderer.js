...
const initializeMarkdownParser = acceptableHighlightValues => {
  ...
  function customHighlight(md, acceptableHighlightValues) {
    ...
    function tokenize(state, silent) {
      ...
      token.meta = { isValidHighlight: acceptableHighlightValues && acceptableHighlightValues.includes(match[1]) }
  ...
  markdown.renderer.rules.highlight = function (tokens, idx) {
    ...
    if (isValidHighlight) {
      return `<span class="bg-gray-200 italic text-emerald-400 px-1 py-0.5 rounded-md break-word">{ ${content} }</span>`;
    } else {
      ...
      return (
        `<span class="bg-red-200 italic text-red-700 px-1 py-0.5 rounded-md inline-flex items-center w-fit gap-x-1 break-word">
          { ${content} }
          ${warningIcon}
        </span>`
      )
