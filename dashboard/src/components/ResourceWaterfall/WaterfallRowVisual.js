...
export default function WaterfallRowVisual({ resource, maxTimestamp }) {
  ...
  let ticks = [<div key={0} className='absolute top-0 left-0 h-full w-full border-r border-gray-300 z-0' style={{ width: '0px' }} />];
  while (ticks.length * TIMESTAMP_EVERY_MS < maxTimestamp) {
    ticks.push(
      <div 
        className='absolute top-0 left-0 h-full w-full border-r border-gray-300 z-0' 
        key={ticks.length} 
        style={{
          width: '0px',
    ...
    <div className='h-full w-full'>
      <div className={`flex items-center z-10 rounded h-full`} 
        ...
        {resource.request_duration ? (
          <>
            <div key='waiting' className={`h-[40%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.waiting[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.waiting[0]}`} 
                  style={{ width: `${resource.waiting_duration * PIXELS_PER_MS}px` }} />
            <div key='dns' className={`h-[40%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.dns[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.dns[0]}`} 
                  style={{ width: `${resource.dns_lookup_duration * PIXELS_PER_MS}px` }} />
            <div key='tls' className={`h-[40%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.tcp[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.tcp[0]}`} 
                  style={{ width: `${(resource.tcp_duration + timings.tls) * PIXELS_PER_MS}px` }} />
            <div key='ssl' className={`h-[40%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.tls[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.tls[0]}`} 
                  style={{ width: `${resource.ssl_duration * PIXELS_PER_MS}px` }} />
            <div key='request' className={`h-[100%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.request[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.request[0]}`} 
                  style={{ width: `${resource.request_duration * PIXELS_PER_MS}px` }} />
            <div key='response' className={`h-[100%] animate-[all] inline-block ${isBeingHovered ? REQUEST_LIFE_CYCLE_COLOR_DICT.response[1] : REQUEST_LIFE_CYCLE_COLOR_DICT.response[0]}`} 
                    ...
                    {resource.response_duration ? (
                      <>
                        ...
                        <tr>
                          <td className='p-2 text-gray-500 flex items-center'>
                            ...
                            <span className='italic'>DNS Lookup</span>
                          </td>
                        ...
                        <tr>
                          <td className='p-2 text-gray-500 flex items-center'>
                            ...
                            <span className='italic'>TCP</span>
                          </td>
                        ...
                        <tr>
                          <td className='p-2 text-gray-500 flex items-center'>
                            ...
                            <span className='italic'>SSL</span>
                          </td>
                        ...
                        <tr>
                          <td className='p-2 text-gray-500 flex items-center'>
                            ...
                            <span className='italic'>Request</span>
                          </td>
                        ...
                        <tr>
                          <td className='p-2 text-gray-500 font-semibold flex items-center'>
                            <span className='italic'>Total Duration</span>
                          </td>
                      ...
                      <tr>
                        <td className='p-2 text-gray-500 flex items-center'>
                          ...
                          <span className='italic'>Network time</span>
                        </td>
