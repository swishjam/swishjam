...
export class DeviceDetails {
  constructor() {
    ...
    this.browserData = uaParser.getResult();
  }
  ...
  all() {
    return {
      ...
      browser_major_version: this.getBrowserMajorVersion(),
      os: this.getOS(),
      os_version: this.getOSVersion(),
      device: this.getDevice(),
      device_type: this.getDeviceType(),
      device_vendor: this.getDeviceVendor(),
  ...
  getOSVersion() {
    return this.browserData.os.version;
  }
  ...
  getDeviceType() {
    return this.browserData.device.type;
  }

  getDeviceVendor() {
    return this.browserData.device.vendor;
  }

  isMobile() {
    // detectmobilebrowsers.com JavaScript Mobile Detection Script
    var dataString = this.browserData.ua || navigator.vendor || window.opera;
  ...
  hasSessionStorage() {
    return this._hasSessionStorage ?? (function () {
      try {
        window.sessionStorage.setItem('swishjam-test', 'test');
        window.sessionStorage.removeItem('swishjam-test');
        return this._hasSessionStorage = true;
      } catch (e) {
        return this._hasSessionStorage = false;
      }
    })()
  }
