module ClickHouseQueries
  module SessionReferrerCountByAnalyticsFamilyAndHour
    class List
      extend ClickHouseQueries::Helpers

      def self.sql(api_key:, start_time:, end_time:, analytics_family:, limit: 10)
        statement = <<~SQL
          SELECT SUM(count) AS count, CONCAT(referrer_url_host, referrer_url_path) AS referrer
          FROM #{Analytics::SessionReferrerCountByAnalyticsFamilyAndHour.table_name}
          WHERE
