  ...
  module Organizations
    module Users
      class ListIds
        ...
        def initialize(public_keys, org_profile_id:, limit: 100)
        ...
        def get_all_user_ids_associated_with_org
          Analytics::Event.find_by_sql(sql).map(&:swishjam_user_id)
        end
        ...
        def sql
          <<~SQL
            SELECT 
              ...
              COUNT(DISTINCT(argMax(user_identifies.swishjam_user_id, user_identifies.occurred_at))) AS num_unique_user_ids
            ...
            WHERE 
              organization_identifies.swishjam_organization_id = '#{@org_profile_id}' AND
