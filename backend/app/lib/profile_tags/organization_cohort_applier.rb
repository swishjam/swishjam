module ProfileTags
  class OrganizationCohortApplier
    ...
    def update_cohort_profile_tags!
      user_ids_in_cohort = recursively_get_user_ids_for_cohort
      current_user_ids_with_cohort_profile_tag = workspace.analytics_user_profiles.joins(:profile_tags).where(profile_tags: { cohort_id: cohort.id, removed_at: nil }).pluck(:id)
      user_ids_to_add_profile_tag = user_ids_in_cohort - current_user_ids_with_cohort_profile_tag
      user_ids_to_remove_profile_tag = current_user_ids_with_cohort_profile_tag - user_ids_in_cohort
      update_cohort_profile_tags_and_enqueue_events!(user_ids_to_add_profile_tag, user_ids_to_remove_profile_tag)
      @data_sync.completed!
      {
    ...
    def update_cohort_profile_tags_and_enqueue_events!(user_ids_to_add_profile_tag, user_ids_to_remove_profile_tag)      
    ...
    def recursively_get_user_ids_for_cohort(user_ids: [], page: 1)
      results = get_user_ids_for_cohort_page(page)
      user_ids.concat(results[:users].map { |u| u['swishjam_user_id'] })
      return user_ids if page >= results[:total_num_pages]
      recursively_get_user_ids_for_cohort(user_ids: user_ids, page: page + 1)
    end

    def get_user_ids_for_cohort_page(page)
