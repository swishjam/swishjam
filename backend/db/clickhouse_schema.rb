...
ClickhouseActiverecord::Schema.define(version: 2023_08_25_161953) do
  ...
  # SQL: CREATE TABLE swishjam_analytics_dev.user_identify_events ( `swishjam_api_key` String, `uuid` String, `device_identifier` String, `swishjam_user_id` String, `occurred_at` DateTime, `ingested_at` DateTime ) ENGINE = MergeTree ORDER BY (swishjam_api_key, toMonth(occurred_at)) SETTINGS index_granularity = 8192
