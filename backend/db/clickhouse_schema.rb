...
ClickhouseActiverecord::Schema.define(version: 2024_01_20_213311) do
...
#   Unknown type 'Enum8('user' = 1, 'organization' = 2)' for column 'swishjam_owner_type'
  ...
  # TABLE: events
  # SQL: CREATE TABLE swishjam_analytics_dev.events ( `uuid` String, `swishjam_api_key` LowCardinality(String), `name` LowCardinality(String), `user_profile_id` UUID, `properties` String, `user_properties` String, `ingested_at` DateTime64(3) DEFAULT now(), `occurred_at` DateTime64(3) ) ENGINE = ReplacingMergeTree(occurred_at) ORDER BY (swishjam_api_key, name, occurred_at, uuid) SETTINGS index_granularity = 8192
