...
RSpec.describe Ingestion::UserIdentifiesIngestion do
  ...
  describe '#ingest!' do
    it 'creates a new analytics_user_profile in Postgres, and a new swishjam_user_profile and user_identify_event in ClickHouse when a new user is provided' do
      ...
      fill_queue_with([
        { uuid: '123', swishjam_api_key: @api_key_1, name: 'identify', occurred_at: timestamp, properties: { userIdentifier: 'some-unique-user', email: 'collin@swishjam.com', first_name: 'Collin', last_name: 'Schneider', some_other_attribute: 'a value!', device_identifier: 'device!' }.to_json},
        { uuid: '456', swishjam_api_key: @api_key_2, name: 'identify', occurred_at: timestamp, properties: { userIdentifier: 'some-unique-user-2', email: 'collin-2@swishjam.com', first_name: 'Collin-2', last_name: 'Schneider-2', 'some_other_attribute-2': 'a value!', device_identifier: 'device-2!' }.to_json},
      ])
    ...
    it 'updates the analytics_user_profile in Postgres, and creates a new user_identify_event in ClickHouse when a user already exists for the unique identifier' do
      IngestionBatch.destroy_all # sigh
      Analytics::SwishjamUserProfile.destroy_all # sigh
      Analytics::UserIdentifyEvent.destroy_all # sigh
      AnalyticsUserProfile.destroy_all # sigh
    ...
    it 'pushes the records into the DLQ if it fails' do
      IngestionBatch.destroy_all # sigh
      expect(Ingestion::QueueManager).to receive(:push_records_into_queue)
                                          .with(Ingestion::QueueManager::Queues.IDENTIFY_DEAD_LETTER_QUEUE, [{ 'invalid_identify_record' => 'foo' }])
                                          .exactly(1).times
      ...
      expect(IngestionBatch.first.num_records).to be(1)
      expect(IngestionBatch.first.event_type).to eq('user_identify')
      # it doesnt have an error message currently, it just gets pushed into the DLQ
