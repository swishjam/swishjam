...
describe EventTriggers::ResendEmailInvoker do
  ...
  describe '#invoke_or_schedule_email_delivery_if_necessary!' do
    ...
    it 'does trigger a Resend email when the `send_once_per_user` config is set to true and this event_trigger has only sent emails to different email addresses' do
      @event_trigger_step.config['send_once_per_user'] = false
      prepared_event = build_new_prepared_event
      prepared_event_2 = build_new_prepared_event(
        uuid: '2', 
        properties: { website_link: 'https://a-different-link-example.com' },
    ...
    it 'delivers the Resend email when the `delay_delivery_by_minutes` config is present, but the `delayed_delivery_at` attribute is present in the `triggered_event_trigger_step.trigger_payload` indicating it was already delayed and this time around it should be delivered' do
      @event_trigger_step.config['delay_delivery_by_minutes'] = 5
      @event_trigger_step.save!
      prepared_event = build_new_prepared_event
    ...
    it 'does not deliver the Resend email when the `delay_delivery_by_minutes` config is present and the `delayed_delivery_at` attribute is present in the `triggered_event_trigger_step.trigger_payload` but the `scheduled_delivery_canceled_at` config is present' do
      @event_trigger_step.config['delay_delivery_by_minutes'] = 5
      @event_trigger_step.save!
      prepared_event = build_new_prepared_event
      
      expect_no_resend_api_request('a_resend_api_key!', anything())
      ...
      # canceling the scheduled delivery after it was scheduled
      result.triggered_event_trigger.cancel!("johnny-canceller@swishjam.com")
      ...
      expect(result.triggered_payload['scheduled_delivery_for']).to be(nil)
      expect(result.triggered_payload['was_scheduled']).to be(true)
      expect(result.error_message).to eq("Scheduled delivery was canceled by johnny-canceller@swishjam.com.")
      expect(result.triggered_payload['scheduled_delivery_canceled_at']).to_not be(nil)
    end
  end
end